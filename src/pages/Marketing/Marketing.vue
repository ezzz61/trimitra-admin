<template>
  <b-container fluid>
        ...
        <div class="row justify-content-center">
          <div class="col-6">
            <button
              ...
              @click="$router.push({ name: 'marketing_add' })"
            >
              Add Marketing
            </button>
          </div>
    ...
    >
      <pre>
are you sure want to delete <strong>{{ infoModal.title }} </strong>from Marketing list ?</pre>
    </b-modal>
  </b-container>
</template>
...
export default {
  ...
  methods: {
    ...
    async handleOk(id) {
      ...
      try {
        let res = await marketingApi.Delete(id);
    ...
    async loadStart() {
      try {
        this.isLoading = true;
        let res = await marketingApi.Get();
        const marketing = res.data.data.data;
        let filtreMarketing = [];
        // filter created
        marketing.map((data) => {
